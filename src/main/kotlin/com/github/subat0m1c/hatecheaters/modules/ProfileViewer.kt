...
object ProfileViewer : Module(
    ...
    val main by ColorSetting("Main", default = Color.DARK_GRAY, false, description = "Main color (primarily background).").withDependency { themes == themesList.lastIndex }
    val accent by ColorSetting("Accent", default = Color.BLUE, true, description = "Accent color (primarily outlines).").withDependency { themes == themesList.lastIndex }
    val font by ColorSetting("Font", default = Color.WHITE, true, description = "Font Color (only affects items without vanilla color codes).").withDependency { themes == themesList.lastIndex }
    val items by ColorSetting("Items", default = Color.GRAY, true, description = "Background color of items").withDependency { themes == themesList.lastIndex }
    val line by ColorSetting("Line", default = Color.BLACK, true, description = "Line Color (primarily separators).").withDependency { themes == themesList.lastIndex }
    ...
    val selected by ColorSetting("Selected", default = Color.CYAN.withAlpha(0.8f), true, description = "Color for selected buttons.").withDependency { themes == themesList.lastIndex }

...
object JsonParseUtils {
    ...
    private suspend fun getHypixelProfile(name: String, uuid: String, profileId: String? = null): Result<PlayerInfo> = runCatching {
        val inputStream = getInputStream(name, "$apiServer$uuid") ?: return Result.failure(FailedToGetHypixelException("Couldn't get Hypixel API input stream!"))
    ...
    private suspend fun getSkyCryptProfile(name: String, uuid: String, profileId: String?): Result<PlayerInfo> = runCatching {
        val inputStream = getInputStream(name, "https://sky.shiiyu.moe/api/v2/profile/${name}") ?: return Result.failure(FailedToGetSkyCryptException("Could not get SkyCrypt input stream. ($name may not have joined hypixel!)"))
        val jsonString = inputStream.bufferedReader().use { it.readText() }
